...
public class HouseService {
    ...
    private HouseResponse buildHouseResponse(House house) {
        ...
        List<HouseResponse.ResidentInfo> residents = getResidentInformation(house);
    ...
    private List<HouseResponse.ResidentInfo> getResidentInformation(House house) {
        try {
            List<EmployeeResponse> employees = employeeClient.getEmployeesByHouseId(house.getId().toString());
            return employees.stream()
                .map(emp -> new HouseResponse.ResidentInfo(
                    emp.getFirstName() + " " + emp.getLastName(),
                    emp.getPhoneNumber(),
                    emp.getEmail(),
                    null // Car information if needed
                ))
                .collect(Collectors.toList());
        } catch (Exception e) {
            log.error("Error getting residents for house: " + house.getId(), e);
    ...
    private Integer getCurrentResidentCount(House house) {
        try {
            return employeeClient.getResidentCountByHouseId(house.getId().toString());
        } catch (Exception e) {
            log.error("Error getting resident count for house: " + house.getId(), e);
            return 0;
        }
    }
    ...
    private String getEmployeeName(Integer employeeId) {
        try {
            return employeeClient.getEmployeeFullName(employeeId.toString());
        } catch (Exception e) {
            log.error("Error getting employee name for ID: " + employeeId, e);
